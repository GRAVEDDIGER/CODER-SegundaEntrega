...
export class PassportAuthService {
    ...
    async findByUserName (username:string){
        try{
            const data= await this.dbService.model.findOne({username})
            if (data !== null){
    ...
    async createUser(username:string,password:string,role:zodCreateUserType["body"]["role"]){
        try {
           const data = await this.dbService.model.create({username,role,password})
           if (data !== null){
            return new ResponseObject(null,true,data.toObject())
